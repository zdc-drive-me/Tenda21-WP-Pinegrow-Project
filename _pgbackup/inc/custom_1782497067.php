...
add_filter( 'pg_query_args', function( $args, $block_id ) {
     ...
     * Activates PG_Blocks_v7::loop_start() if Pinegrow generates it for facilitators-grid.
     */
     ...
     * Activates PG_Blocks_v7::loop_start() if Pinegrow generates it for experiences-grid.
     */
...
add_filter( 'render_block', function( $block_content, $block ) {
    ...
    $events_view = isset( $_GET['events_view'] ) ? sanitize_key( wp_unslash( $_GET['events_view'] ) ) : '';
    $query_args  = array(
        'post_type'      => 'event',
        'post_status'    => 'publish',
        'posts_per_page' => -1,
    ...
    <section <?php echo $wrapper_attr; ?>>
        <div class="max-w-6xl mx-auto w-full">
            <?php
$current_view      = isset( $_GET['events_view'] ) ? sanitize_text_field( wp_unslash( $_GET['events_view'] ) ) : '';
$is_all_view       = ( 'all' === $current_view );
$events_archive_url = get_post_type_archive_link( 'event' );
$upcoming_url      = remove_query_arg( 'events_view', $events_archive_url );
$all_events_url    = add_query_arg( 'events_view', 'all', $events_archive_url );
?>
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between border-b border-mist-400 pb-4 mb-10 text-xs font-sans uppercase tracking-[0.2em] text-charcoal-500">
                <div class="flex flex-col gap-2 md:flex-row md:items-center md:gap-4">
                    <?php if ( $schedule_label ) : ?>
                        <span><?php echo esc_html( $schedule_label ); ?></span>
                    <?php endif; ?>
                    <nav aria-label="<?php esc_attr_e( 'Events filter', 'tenda21' ); ?>" class="flex items-center gap-2 text-[0.6rem] tracking-[0.2em]">
                        <a href="<?php echo esc_url( $upcoming_url ); ?>" class="<?php echo $is_all_view ? 'text-charcoal-600 hover:text-charcoal-900' : 'text-charcoal-900 underline underline-offset-4'; ?>"><?php esc_html_e( 'Upcoming Events', 'tenda21' ); ?></a>
                        <span class="text-mist-500">/</span>
                        <a href="<?php echo esc_url( $all_events_url ); ?>" class="<?php echo $is_all_view ? 'text-charcoal-900 underline underline-offset-4' : 'text-charcoal-600 hover:text-charcoal-900'; ?>"><?php esc_html_e( 'All Events', 'tenda21' ); ?></a>
                    </nav>
                </div>
