...
add_filter( 'pg_query_args', function( $args, $block_id ) {
     ...
     * Returns: all upcoming Events by default, or all Events when events_view=all
     * Ordered: chronologically ascending by event_start_date
     */
    } elseif ( $block_id === 'events-schedule' ) {
        $view = isset( $_GET['events_view'] ) ? sanitize_text_field( wp_unslash( $_GET['events_view'] ) ) : 'upcoming';
...
add_filter( 'render_block', function( $block_content, $block ) {
    ...
    $upcoming_filter_label = isset( $attrs['events_filter_upcoming_label'] ) && $attrs['events_filter_upcoming_label'] !== null
        ? $attrs['events_filter_upcoming_label']
        : __( 'Upcoming Events', 'tenda21' );
    $all_filter_label      = isset( $attrs['events_filter_all_label'] ) && $attrs['events_filter_all_label'] !== null
        ? $attrs['events_filter_all_label']
        : __( 'All Events', 'tenda21' );
    ...
    $current_view_raw      = isset( $_GET['events_view'] ) ? sanitize_text_field( wp_unslash( $_GET['events_view'] ) ) : '';
    $filter_view           = $current_view_raw === 'all' ? 'all' : 'upcoming';
    $current_url           = home_url( wp_unslash( $_SERVER['REQUEST_URI'] ?? '/' ) );
    $base_filter_url       = remove_query_arg( array( 'events_view', 'paged' ), $current_url );
    $upcoming_url          = $base_filter_url;
    $all_events_url        = add_query_arg( 'events_view', 'all', $base_filter_url );
    ...
    <section <?php echo $wrapper_attr; ?>>
        <div class="max-w-6xl mx-auto w-full">
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between border-b border-mist-400 pb-4 mb-6 text-xs font-sans uppercase tracking-[0.2em] text-charcoal-500">
            ...
            <div class="mb-10 flex flex-wrap items-center justify-between gap-3">
                <span class="text-xs font-sans uppercase tracking-[0.2em] text-charcoal-500"><?php esc_html_e( 'Show', 'tenda21' ); ?></span>
                <div class="inline-flex items-center rounded-full border border-mist-400 bg-bone-50 p-1">
                    <a href="<?php echo esc_url( $upcoming_url ); ?>" class="<?php echo $filter_view === 'upcoming' ? 'inline-flex items-center rounded-full bg-charcoal px-4 py-2 text-xs font-sans uppercase tracking-[0.16em] text-bone-100' : 'inline-flex items-center rounded-full px-4 py-2 text-xs font-sans uppercase tracking-[0.16em] text-charcoal-600 hover:text-charcoal-900'; ?>"> <?php echo esc_html( $upcoming_filter_label ); ?> </a>
                    <a href="<?php echo esc_url( $all_events_url ); ?>" class="<?php echo $filter_view === 'all' ? 'inline-flex items-center rounded-full bg-charcoal px-4 py-2 text-xs font-sans uppercase tracking-[0.16em] text-bone-100' : 'inline-flex items-center rounded-full px-4 py-2 text-xs font-sans uppercase tracking-[0.16em] text-charcoal-600 hover:text-charcoal-900'; ?>"> <?php echo esc_html( $all_filter_label ); ?> </a>
                </div>
            </div>
