...
if ( ! function_exists( 'tenda21_enqueue_scripts' ) ) :
    ...
    wp_enqueue_style( 'tenda21-tailwind', get_template_directory_uri() . '/tailwind_theme/tailwind.css', [], '1.0.114', 'all');

    wp_deregister_style( 'tenda21-style' );
    wp_enqueue_style( 'tenda21-style', get_bloginfo('stylesheet_url'), [], '1.0.114', 'all');
...
if ( ! function_exists( 'tenda21_require_pg_blocks_helper' ) ) {
    function tenda21_require_pg_blocks_helper() {
        if ( ! class_exists( 'PG_Blocks_v7' ) ) {
            require_once "inc/wp_pg_blocks_helpers.php";
        }
    }
}
tenda21_require_pg_blocks_helper();
